...
def test_list_kip_files(runner: KipCliRunner, fake_config: Config) -> None:
    result = runner.invoke_config(["list"])
...
def test_add_kip_file(
    fake_config: Config, runner: KipCliRunner, tmp_path: pathlib.Path
) -> None:
    new_config_file = tmp_path / "commands.yaml"
    new_config_file.touch()
    result = runner.invoke_config(["add", str(new_config_file)])
    assert result.exit_code == 0
    assert str(new_config_file) in fake_config.kip_files


def test_add_kip_file_file_exists(runner: KipCliRunner) -> None:
    result = runner.invoke_config(["add", "/tmp/this/file/does/not/exist.yaml"])
    assert result.exit_code == 2
    assert "exist" in result.stdout


def test_add_kip_file_file_is_not_dir(
    runner: KipCliRunner, tmp_path: pathlib.Path
) -> None:
    dir_to_add = tmp_path / "dir/"
    dir_to_add.mkdir()
    result = runner.invoke_config(["add", str(dir_to_add)])
    assert result.exit_code == 2
    assert "directory" in result.stdout


def test_add_kip_file_relative_path_is_converted_to_absolute(
    fake_config: Config, runner: KipCliRunner
) -> None:
    cur_file = pathlib.Path(__file__).relative_to(pathlib.Path().absolute())
    result = runner.invoke_config(["add", str(cur_file)])
    assert result.exit_code == 0
    assert str(cur_file.absolute()) in fake_config.kip_files


def test_remove_kip_file(fake_config: Config, runner: KipCliRunner) -> None:
    result = runner.invoke_config(["remove", "/tmp/dir/test-commands.yaml"])
    assert result.exit_code == 0
...
def test_remove_kip_file_relative_path_is_converted_to_absolute(
    ...
    result = runner.invoke_config(["remove", str(rel_path_to_cur_file)])
    assert result.exit_code == 0
