...
def test_list_files(cli_runner: CliRunner, fake_config: Config) -> None:
    result = cli_runner.invoke(app, ["config", "list-files"])
...
def test_add_file(
    ...
    result = cli_runner.invoke(app, ["config", "add-file", str(new_config_file)])
    assert result.exit_code == 0
    assert str(new_config_file) in fake_config.kip_files


def test_add_file_file_exists(cli_runner: CliRunner) -> None:
    result = cli_runner.invoke(
        app, ["config", "add-file", "/tmp/this/file/does/not/exist.yaml"]
    )
    assert result.exit_code == 2
    assert "exist" in result.stdout


def test_add_file_file_is_not_dir(
    cli_runner: CliRunner, tmp_path: pathlib.Path
) -> None:
    dir_to_add = tmp_path / "dir/"
    dir_to_add.mkdir()
    result = cli_runner.invoke(app, ["config", "add-file", str(dir_to_add)])
    assert result.exit_code == 2
    assert "directory" in result.stdout


def test_add_file_relative_path_is_converted_to_absolute(
    fake_config: Config, cli_runner: CliRunner
) -> None:
    cur_file = pathlib.Path(__file__).relative_to(pathlib.Path().absolute())
    result = cli_runner.invoke(app, ["config", "add-file", str(cur_file)])
    assert result.exit_code == 0
    assert str(cur_file.absolute()) in fake_config.kip_files


def test_remove_file(fake_config: Config, cli_runner: CliRunner) -> None:
    result = cli_runner.invoke(
        app, ["config", "remove-file", "/tmp/dir/test-commands.yaml"]
    )
    assert result.exit_code == 0
    assert fake_config.kip_files == ["/tmp/another/dir/commands.yaml"]


def test_remove_file_relative_path_is_converted_to_absolute(
    fake_config: Config, cli_runner: CliRunner
) -> None:
    ...
    result = cli_runner.invoke(
        app, ["config", "remove-file", str(rel_path_to_cur_file)]
    )
    assert result.exit_code == 0
